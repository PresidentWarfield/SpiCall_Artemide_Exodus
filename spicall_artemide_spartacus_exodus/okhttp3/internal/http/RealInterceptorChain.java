  ...
  implements Interceptor.Chain
{
  ...
  public Response proceed(Request paramRequest, StreamAllocation paramStreamAllocation, HttpCodec paramHttpCodec, RealConnection paramRealConnection)
  {
    if (this.index < this.interceptors.size())
    {
      ...
      paramRealConnection = new RealInterceptorChain(this.interceptors, paramStreamAllocation, paramHttpCodec, paramRealConnection, this.index + 1, paramRequest, this.call, this.eventListener, this.connectTimeout, this.readTimeout, this.writeTimeout);
      paramRequest = (Interceptor)this.interceptors.get(this.index);
      paramStreamAllocation = paramRequest.intercept(paramRealConnection);
      if ((paramHttpCodec != null) && (this.index + 1 < this.interceptors.size()) && (paramRealConnection.calls != 1))
      {
      ...
      {
        if (paramStreamAllocation.body() != null) {
          return paramStreamAllocation;
        }
        paramStreamAllocation = new StringBuilder();
...
/* Location:              ~/okhttp3/internal/http/RealInterceptorChain.class
 *
 * Reversed by:           J
 */